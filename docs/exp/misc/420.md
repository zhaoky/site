...
**核心原理**: 通过修改系统 hosts 文件 (`/etc/hosts`) 实现本地 DNS 解析劫持,将域名映射到指定 IP 地址,绕过 DNS 查询过程。

**主要作用**: 快速切换不同的 hosts 配置方案,无需手动编辑系统文件。
...
2. **域名预解析**: 在 DNS 生效前测试新域名

   ```text
   101.200.xx.xx new-domain.com
   ```

3. **屏蔽广告/追踪**: 将广告域名指向无效地址
...
- **DNS 优先级**: `hosts 文件 > DNS 解析 > 代理规则`
- **配合 Whistle**: hosts 映射到本地后,Whistle 可继续拦截处理
- **配合 Clash**: hosts 解析完成后,流量仍会经过 Clash 的代理规则

**使用流程**:

```mermaid
flowchart LR
    A[应用发起请求<br>api.example.com] --> B{查询hosts文件}
    B --> |找到映射| C[直接使用<br>127.0.0.1]
    B --> |未找到| D[DNS查询]
    C --> E[发起TCP连接]
    D --> E
    E --> F[后续代理处理<br>Whistle/Clash]
...
- SwitchHosts 需要管理员权限才能修改 hosts 文件
- macOS 可能需要刷新 DNS 缓存: `sudo dscacheutil -flushcache`
- Chrome 有独立的 DNS 缓存,可在 `chrome://net-internals/#dns` 清除
- hosts 文件不支持端口配置,仅能映射 IP
...
**目标**: 使用真实域名访问本地开发服务,无需修改代码中的 API 地址。
...
1. **SwitchHosts 配置**:
   ...
   127.0.0.1 payment.example.com
   ```

   启用该方案后,访问 `http://dev.example.com:3000` 即可访问本地开发服务器。

2. **配合 Whistle 深度调试**:

   SwitchHosts 解析域名到 `127.0.0.1` 后,在 Whistle 中配置:

   ```text
   # API Mock
   api.example.com/user resBody://{{"id": 1, "name": "测试"}}

   # 请求转发到本地不同端口
   api.example.com 127.0.0.1:8080
   auth.example.com 127.0.0.1:8081
   ```

3. **多环境快速切换**:

   在 SwitchHosts 中创建多个方案:
...
4. **https 本地开发**:
   ...
   配合 Whistle 的证书,实现 HTTPS 本地开发:

   - SwitchHosts: `127.0.0.1 secure.example.com`
   - Whistle 规则: `secure.example.com 127.0.0.1:3000`
   - 访问 `https://secure.example.com` 即可,Whistle 自动处理证书
